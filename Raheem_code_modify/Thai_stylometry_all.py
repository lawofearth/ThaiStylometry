...
def get_set_ids(book_name):
    ...
    doc = []
    line = linecache.getlines(book_name)
    ...
    print("query set is ", querySet[:10])
...
def NormalizeOneDem(oneDemdata):
    # cant pickle except top level module for multi processing
    maxnum = -1000
    minnum = 100000
...
def queryExp(q):
    ...
    data += (str(LSHMHDLongListTime) + "\n")
    start = time.time()
    sortedListLSH = M2HDPrunList(queryParaList, ListLSH_ALL, combinedhitQP, datasetm, paraIndex, R,
                                 hitparaIndex,
                                 doc_hit_above_T_dict, "lsh", startp, endp)
    LSH_M2HD_Result, LSH_M2HD_values = geneM2HDPrun(sortedListLSH, queryParaList, combinedhitQP, topknn,
                                                    flagNum,
    ...
    def print_authorid(final_list):
        if len(final_list) == 1:
            ...
            print "heeeee", doc_to_au_dict[final_list[1]]
    ...
    LSH_SHD_PKNN = PKNN(queryParaList, LSH_SHD_list, "LSH_SHD", "", 20, 0, combinedhitQP, 5, datasetP,
                        paraIndex)
    print "LSH_SHD PKNN: ", LSH_SHD_PKNN
    data += (str(LSH_SHD_PKNN) + "\n")

    LSH_MHD_PKNN = PKNN(queryParaList, LSH_MHD_list, "LSH_MHD", "", 20, 0, combinedhitQP, 5, datasetP,
                        paraIndex)
    print "LSH_MHD PKNN: ", LSH_MHD_PKNN
    data += (str(LSH_MHD_PKNN) + "\n")

    LSH_M2HD_PKNN = PKNN(queryParaList, LSH_MHD_list, "LSH_M2HD", "", 20, 0, combinedhitQP, 5, datasetP,
                         paraIndex)
    ...
    for f_i in f:
        for k_i in k:
            ...
            for b_i in book:
                ...
                startp = 0.25  # default 0.25
                endp = 0.50  # default 0.50
                ...
                print "para2doc", para_to_doc_dict
                ...
                print "\n\ndoc_to_para_dict", doc_to_para_dict, "\n\n"
                ...
                print 'AAAAAAAAAAAAAAAAAAAA'#, combinedresultQ
                ...
                for qq in querySet:
                    print "qq: ", qq
                    print "doc_to_para_dict: ", doc_to_para_dict
                    in_q.append(doc_to_para_dict[qq])
        ...
        print(output_file)
        output_file.to_csv(output_name + ".csv")
